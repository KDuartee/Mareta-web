...
const ingredientSlug = (ingredient) => ingredient.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
const ingredientImagePath = (step, ingredient) => `assets/ingredients/${['bases', 'proteins', 'toppings', 'dressings', 'crunch'][step]}/${ingredientSlug(ingredient)}.jpg`;
...
function renderBuilder() {
  ...
  options.innerHTML = `<h3>${title}</h3><p class="muted">${instructions}</p><div class="pills">${values.map((value) => { const label = currentLang === 'en' ? (ingredientEnglish[value] || value) : value; return `<button class="pill ingredient-option ${isSelected(value) ? 'selected' : ''}" type="button" aria-pressed="${isSelected(value)}" data-option="${value}"><img class="ingredient-thumb ingredient-photo" src="${ingredientImagePath(builderStep, value)}" alt="" loading="lazy" decoding="async" /><span class="ingredient-label">${label}</span><span class="pill-check" aria-hidden="true">${isSelected(value) ? '✓' : ''}</span></button>`; }).join('')}</div>${builderStep < 4 ? `<button class="next-step" type="button">${copy.next}</button>` : ''}`;
